        ...
        while (true)
        {
            ...
            if (!msg.empty())
            {
                ...
                else if (msg_type == "创建用户")
                {
                    if (msg_server == "qq")
                    {
                        dtm::qq_user_DTM temp_user;
                        temp_user.from_json(msg["用户结构体"]);
                        all_server[msg_server].first.emplace_back(temp_user);
                    }
                    else if (msg_server == "weixin")
                    {
                        dtm::weixin_user_DTM temp_user;
                        temp_user.from_json(msg["用户结构体"]);
                        all_server[msg_server].first.emplace_back(temp_user);
                    }
                }
